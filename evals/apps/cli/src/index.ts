...
const run = async (toolbox: GluegunToolbox) => {
	...
	if (id) {
		...
		run = await createRun({
			...
			pid: process.pid,
			socketPath: path.resolve(os.tmpdir(), `roo-vibecoding-evals-${crypto.randomUUID().slice(0, 8)}.sock`),
		})
	...
	await execa({ cwd: exercisesPath })`git config user.name "Roo Code Chinese"`
